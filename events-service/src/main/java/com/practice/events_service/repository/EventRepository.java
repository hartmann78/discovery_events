...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @Query(value = """
            ...
            where (cast(?1 as anyarray) is null or initiator_id in (?1))
              ...
              and ((cast(?4 as timestamp without time zone) is null and cast(?5 as timestamp without time zone) is null)
                or (cast(?4 as timestamp without time zone) is not null and cast(?5 as timestamp without time zone) is not null
    ...
    @Query(value = """
            ...
            """,
            nativeQuery = true)
    List<Event> publicGetPublishedEvents(String text, Long[] categories, Boolean paid, LocalDateTime rangeStart, LocalDateTime rangeEnd,
